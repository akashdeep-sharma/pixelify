...
export async function getAllMetfieldsValues(data) {
  ...
  try {
    ...
    const response = await clientRes.get({
      path: 'metafields',
      query: { key: 'pixelify', namespace: 'IDs' },
    });

    return response.body.metafields[0].value;
  } catch (e) {
    console.log(`${e} session info :{${sessions}}`);
  }
}
...
async function addScriptIntoTheme(data) {
  ...
  var updatedThemeCode = themeCode.replace(
    '<head>',
    "<head>{% render 'MultiPixelPixelify' %}"
  );
  ...
  const response4 = await clientRes.put({
    // add new snippet with pixel code
    path: `themes/${response.body.themes[0].id}/assets`,
    data: {
      asset: {
